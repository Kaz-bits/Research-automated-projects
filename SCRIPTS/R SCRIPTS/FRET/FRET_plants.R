...
# Selección de biosensores específicos
# Es necesario ejecutar la función "FRET.data" para 
# generar las carpetas de los biosensores correspondientes

FRET.plants <- function(dir.bios, bios.list, format.plot = "pdf") {
  
  # Obtener el nombre de cada biosensor
  temp_files <- list.files(dir.bios)
  
  # Seleccionar los biosensores que se desean graficar
  temp_bios <- temp_files[temp_files == bios.list]
  ...
  # Guardar gráfico 
  ggsave(plot = plot, 
         filename = file.path(dir.bios, paste0("all_biosensors_plants.pdf")), 
         device = format.plot, 
         width = 5, height = 15, units = "in", dpi = 450)
