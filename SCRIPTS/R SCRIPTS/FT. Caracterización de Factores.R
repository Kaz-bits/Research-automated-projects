...
fasta <- read.fasta(file = "DATA/DATA FOR R/SEC_O.txt", seqtype = "AA", 
                    as.string = FALSE, set.attributes = FALSE)

#Cargar archivo con rangos de desorden

rangos <- read_tsv(file = "DATA/DATA FOR R/ATG_ranges.txt", 
...
path_FT <- "DATA/DATA FOR R/sequences_d_fasta.txt"
write.fasta(as.list(fasta), names(fasta), path_FT) 
...
TFs <- read.delim("DATA/DATA FOR R/TFs_arabidopsis.txt")
...
#Se econtraron 1959 AGIs de los 1982 TFs en la base de datos de 27411. 
