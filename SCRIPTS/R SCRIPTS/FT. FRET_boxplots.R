...
fret <- readxl::read_xlsx(path = "DATA/DATA FOR R/FRET/Data_FRET/IDRFT_REP3_P2. ALTA. N1.xlsx", 
                          sheet = 2, col_names = FALSE)[-c(1:9),]
...
fret_boxplot <- function(rep1, rep2, rep3, name_plot, 
  ...
  t1 <- c(rep1:(rep1+12-1), 
          rep2:(rep2+12-1), 
          rep3:(rep3+12-1))[-length(c(rep1:(rep1+12), 
                                      rep2:(rep2+12), 
                                      rep3:(rep3+12)))]
  
  #Prueba t-test para 0M y 0.5M
  p1 <- t.test(fret_all[t1,][fret_all[t1,][,1] == 0,][,6],
               fret_all[t1,][fret_all[t1,][,1] == 0.5,][,6])[3][[1]]
  #Prueba t-test para 0M y 1.0M
  p2 <- t.test(fret_all[t1,][fret_all[t1,][,1] == 0,][,6],
               fret_all[t1,][fret_all[t1,][,1] == 1,][,6])[3][[1]]
  #Prueba t-test para 0M y 1.5M
  p3 <- t.test(fret_all[t1,][fret_all[t1,][,1] == 0,][,6],
               fret_all[t1,][fret_all[t1,][,1] == 1.5,][,6])[3][[1]]
  ...
  plot <- ggplot() + 
    geom_boxplot(data = fret_all[c(rep1:(rep1+12-(1)), 
                 ...
                 fill = c("#53AEF5", "#538CF5", 
                          "#535DF5", "#3E04F2"),
                 outlier.shape = NA) +
    geom_jitter(data = fret_all[c(rep1:(rep1+12-(1)), 
                                  rep2:(rep2+12-(1)), 
                                  rep3:(rep3+12-(1))),], 
                aes(x = Tratamiento, 
                    y = Normalización, group = Tratamiento),
    ...
    annotate(geom = "text", x = 0.75, y = 2.85, label = name_plot,
             size = 5)
  #Guardar gráfico
  if (save_box == TRUE) {
