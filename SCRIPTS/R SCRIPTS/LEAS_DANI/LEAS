...
# Escribir directorio donde se encuentre el archivo de FRET_ratio
# de los constructos
dir_fret <- "/media/kaz-bits/TOSHIBA EXT/FRET_REPTICION/FRET/fret_ratio_all.csv"

# Cargar archivo ----
df_fret <- read.csv(file = dir_fret, header = TRUE, sep = ",")

# Construir modelo no lineal ----

# Obtener los nombres de los constructos del
# archivo cargado
list_names <- names(table(df_fret$Construct))
...
for (a in list_names) {
  ...
  temp <- df_fret[df_fret$Construct == a, ]
...
ggplot() +
  geom_line(data = temp_newdata, 
            ...
            size = 0.7, 
            lty = 1, alpha = 1, show.legend = TRUE) +
  geom_line(data = temp_newdata[temp_newdata$Construct == 201, ], 
            aes(x = (Treatment/1000), y = p, 
                group = Construct),
            size = 0.8, color = "blue", 
            lty = 1, alpha = 1, show.legend = FALSE) +
  theme_classic() +
  labs(x = "[NaCl] (M)", 
       y = "Normalized \nDxAm/DxDm") +
  ...
  scale_color_manual(name = NULL, 
                     values = c("#198F09", "#FD70B4", 
                                "#9370DB", "#1D90FF", 
                                "#00CD66"))

#Guardar gráfico
ggsave(filename = "/media/kaz-bits/TOSHIBA EXT/FRET_REPTICION/FRET/DRC_LEAS.png",
       device = "png", width = 5.5, height = 4, units = "in", 
       dpi = 450) 
